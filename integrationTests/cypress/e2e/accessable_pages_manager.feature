Feature: Tests for the view restriction on some pages with manager rights.
  ...
  Scenario: a user sees the account page after login
    Then the account page for user "manager" is open

  Scenario: a user can see the tabs user, groups, userimport and import overviews in the navbar
    Then users, groups, userimport and import overview should be visible in the navbar
