import {CesIconCheck, CesIconCross} from "@cloudogu/ces-theme-tailwind";
...
function RadioGroupEntry({label, checked, children, className = ""}: {
    label: React.ReactNode;
    checked: boolean;
    children?: React.ReactNode;
    className?: string;
    ...
    return (
        <div className={`flex flex-col gap-0 items-start justify-start self-stretch shrink-0 relative ${className}`} >
            <div className={`border-solid ${checked ? "bg-brand-colors-brand-weaker-brand-100 border-brand-colors-brand-brand-700" : "border-neutral-colors-neutral-weak-neutral-300"} ${hasChildren ? "rounded-tl-rounded-lg rounded-tr-rounded-lg border-t border-r border-l pr-2" : "rounded-rounded-lg border"} flex flex-row gap-0 items-center justify-start self-stretch shrink-0 min-h-[40px] relative overflow-hidden`} >
                {radioButton(label, checked)}
            </div>
            {hasChildren && (
                <div className={`bg-neutral-colors-neutral-0 rounded-br-rounded-lg rounded-bl-rounded-lg border-solid ${checked ? "border-brand-colors-brand-brand-700" : "border-neutral-colors-neutral-weak-neutral-300"} border-r border-b border-l p-6 flex flex-col gap-4 items-end justify-start self-stretch shrink-0 relative overflow-hidden`} >
                    ...
                    <div className="flex flex-row gap-4 gap-y-0 items-center justify-end flex-wrap content-center self-stretch shrink-0 relative" >
                        ...
                        <div className="rounded-rounded-sm border-solid border-brand-colors-brand-brand-700 border-2 pt-2 pr-25 pb-2 pl-25 flex flex-row gap-05 items-center justify-center shrink-0 min-w-[40px] min-h-[40px] relative overflow-hidden" >
                            ...
                            <div className="pr-15 pl-15 flex flex-row gap-0 items-center justify-center shrink-0 relative" >
                                <div className="text-brand-colors-brand-brand-700 text-left font-copy-paragraph-bold-font-family text-copy-paragraph-bold-font-size leading-copy-paragraph-bold-line-height font-copy-paragraph-bold-font-weight relative" >
                                    Auswahl aufheben
                                </div>
                            </div>
                        </div>
...
function RadioGroup({label, children, className = ""}: {
    ...
    return (
        <div className={`flex flex-col gap-1 items-start justify-start self-stretch shrink-0 relative ${className}`} >
...
export function DoguSelection({label}: DoguSelectionProps) {
    return (
        <RadioGroup label={label}>
            <RadioGroupEntry label="Auswahl an Dogus" checked={true}>
